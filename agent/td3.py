...
class TD3Agent(DDPGAgent):
    ...
    def get_Qs(self, 
        ...
        noise = torch.normal(0, self.policy_noise, action.size()).to(self.device)
        noise = torch.clamp(noise, -self.noise_clip, self.noise_clip)
        next_action = self.actor_target(next_state) + noise
        next_action = torch.clamp(next_action, self.actor_net.action_space.low.min().item(), self.actor_net.action_space.high.max().item())
        with torch.no_grad():
            Q_target = reward + self.gamma * (1 - done) * torch.min(self.critic_target(next_state, next_action), self.critic_target_2(next_state, next_action))        
        ############################
